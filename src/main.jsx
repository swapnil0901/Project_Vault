  ...
  useEffect(() => {
    ...
    Promise.all([apiRequest('/api/projects'), apiRequest('/api/tasks'), apiRequest('/api/achievements'), apiRequest('/api/overview-summary'), usersRequest, apiRequest('/api/guidance'), apiRequest('/api/requests'), apiRequest('/api/feedback'), apiRequest('/api/messages'), apiRequest('/api/knowledge'), apiRequest('/api/process')])
      .then(([remoteProjects, remoteTasks, remoteAchievements, remoteOverviewSummary, remoteUsers, remoteGuidance, remoteRequests, remoteFeedback, remoteMessages, remoteKnowledge, remoteProcess]) => {
        ...
        apiRequest('/api/reports').then(setReports).catch(() => setReports([]))
      })
      .catch((error) => {
        if (!cancelled) {
...
function WorkspacePage({ page, projects, tasks, users = [], user, guidance, messages, knowledge, settings, processSteps, searchQuery, selectedProject, onSelectProject, onChat, onNewProject, onEditProject, onDeleteProject, onToggleTask, onRequestProof, onGuidance, onSubmitReport, onSendMessage, onAddKnowledge, onSaveSettings, onSaveProcess, canSeePrivate, onRequestDecision, onAddTask, onUpdateTask, requests = [], requestFeedback = [], reports = [], achievements = [], onAddAchievement, onAddUser }) {
  const pageContent = {
  ...
  if (page === 'Reports') return <ReportsPage projects={projects} tasks={tasks} guidance={guidance} reports={reports} onGuidance={onGuidance} onSubmitReport={onSubmitReport} />
...
function ReportsPage({ projects = [], tasks = [], guidance = [], reports = [], onGuidance, onSubmitReport }) {
  const activeProjects = projects.filter((project) => !project.completed)
  ...
  const getReports = (projectName) => reports.filter((report) => report.projectName === projectName)
